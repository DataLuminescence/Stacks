...
class User:
    ...
    # # gets recipies that are linked to users based on user id
    # @classmethod
    # def get_user_and_recipes_by_user_id(cls,data):
    #     query = """SELECT * FROM users LEFT JOIN recipies 
    #             ON recipies.user_id = users.id
    #             WHERE users.id = %(id)s;"""
    #     result = connectToMySQL(cls.db).query_db(query,data)

    #     if len(result) < 1:
    #         return False
    #     user_and_recipes = cls(result[0])
